...
MART_SALES テーブルのデータを使って、標準化された月次売上分析レポートをマークダウン形式で生成します。
...
FROM with_prev
WHERE SALE_MONTH = (SELECT MAX(DATE_TRUNC('MONTH', TRANSACTION_DATE)) FROM MART_SALES);
```
...
WHERE DATE_TRUNC('MONTH', TRANSACTION_DATE) = (
    SELECT MAX(DATE_TRUNC('MONTH', TRANSACTION_DATE)) FROM MART_SALES
)
GROUP BY CATEGORY
...
WITH monthly AS (
    ...
    WHERE TRANSACTION_DATE >= DATEADD('MONTH', -5,
        (SELECT MAX(TRANSACTION_DATE) FROM MART_SALES))
    GROUP BY DATE_TRUNC('MONTH', TRANSACTION_DATE)
)
SELECT
    SALE_MONTH,
...
```
SQL compilation error: Object 'MART_SALES' does not exist
```
...
```sql
SELECT CHANNEL, COUNT(*) FROM MART_SALES GROUP BY CHANNEL;
```
